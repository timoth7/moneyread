...
export function Onboarding({ open, step, language, onStepChange, onClose }: Props) {
  ...
  return (
    <AnimatePresence>
      {open && (
        <motion.div
          className="fixed inset-0 z-[450] flex items-end justify-center bg-black/55 p-4 pb-[max(24px,calc(16px+env(safe-area-inset-bottom)))] sm:items-center sm:pb-8"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
        ...
        >
          <motion.div
            ...
            exit={{ y: 16, opacity: 0 }}
            className="w-full max-w-md rounded-2xl border-2 border-[var(--color-primary)]/40 bg-[var(--color-surface)] p-6 shadow-2xl"
          >
            ...
            <div className="mt-6 flex flex-wrap gap-2">
              ...
              {step < total ? (
                <button
                  ...
                  className="flex-1 rounded-xl bg-[var(--color-primary)] px-4 py-2.5 text-sm font-bold text-white min-w-[120px]"
                >
                  {o.next}
                </button>
              ) : (
